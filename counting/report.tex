...
checking is done symbolically and uses satisfiablity modulo theories (SMT) solvers.
...
\begin{example} Let  $F \equiv c = \{x \mid 0 < x \le y \land 0 < x \le z\}$. In this case, $c$ can not be directly expressed with an arithmetic expression, as the symbolic value depends on wether $y < z$. However, with a disjonction on $y >z$, $F$ is also equivalent to a pure arithmetic formula $(y > z \implies c = y) \lor (y \le z \implies c = z)$.
\end{example}
...
If the formula is a litteral $l$ of a theory $\mathbf{T}_i$, then we look at the value of $l$ in
...
Let $S$ and $S'$ be two arithmetic domains, and $A$, $A'$ the sets of assumptions associated to
them. To do the intersection of the domains $S \sqcap S'$, we are going to
...
A corallary of property \ref{distincts} is the fact that the cardinality of the reunion of the elements
