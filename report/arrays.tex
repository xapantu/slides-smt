...
At this point we have a set of domains that all have the same intervals
...
$\phi_1, \ldots, \phi_l$. From these, we can create a set of formula
$\psi_1, \ldots, \psi_{l'}$ which is a
...
There can be at most $2^l$ formulas, but of course that can be
...
algorithm then has to take into account the decision that the SMT solver makes
when it outputs the constraints, so that the result is consistent
...
explains how the SMT solver Z3 works.

The only two operations on arrays are \texttt{select} and
\texttt{store}. \texttt{(select\ a\ x)} accesses the array $a$ at index
...
\begin{subequations}
    ...
    Additionaly, there is the extensionality axiom:
...
to take decisions about index terms which appear inside a \texttt{store}
...
(the default value). This example illustrates the fact that there is no need to
...
extracting from the model the slice to which they belong, and group them by
slice. Then, for every array constraint $\phi_\alpha$ of this slice,
some new clauses must be sent to the solver to ensure that the values decided by
the SMT solver are consistent with the number of $x$ which
satisfies $\phi_\alpha$

Equalities between arrays must also be tracked. They must be taken
